...
schools, and government/research hubs.
...
`con` · `meetup` · `maker` · `school` · `gov`

## Editing data

Open `data.js` — the header comment documents the row format. Rules of the
house: coordinates are approximate (city-level is fine), **never guess
contact details** (leave `url`/`address`/`phone` empty instead), and only
set `next:` dates you've verified — they drive the Upcoming panel and
